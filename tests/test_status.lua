...
describe('status.attach', function()
    it('sets winbar to %! expression', function()
        load_status()
        child.lua([[require('slopcode.status').attach(0, 0)]])
        local winbar = child.lua_get('vim.wo[0].winbar')
    ...
    it('shows idle state with slopcode title', function()
        load_status()
        child.lua([[
            require('slopcode.status').attach(0, 0)
...
describe('status.subheader', function()
    it('shows model name in idle winbar', function()
        ...
        child.lua([[
            require('slopcode.status').attach(0, 0)
            require('slopcode.status').subheader('gpt-4o')
        ]])
    ...
    it('clears model when set to empty string', function()
        ...
        child.lua([[
            require('slopcode.status').attach(0, 0)
            require('slopcode.status').subheader('gpt-4o')
            require('slopcode.status').subheader('')
        ]])
...
describe('status.detach', function()
    it('cleans up module state including model', function()
        load_status()
        child.lua([[
            require('slopcode.status').attach(0, 0)
            require('slopcode.status').subheader('gpt-4o')
            require('slopcode.status').detach()
        ]])
        MiniTest.expect.equality(resolve(), '%#Title#  %*')
    end)
end)
